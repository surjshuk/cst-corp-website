...
type CollapisblePointsProps = {
  title?:string;
  data: CollapisblePoint[];
};

export const CollapsiblePoints: React.FC<CollapisblePointsProps> = ({ data,title }) => {
  const [openItems, setOpenItems] = useState<number[]>(data.map((_, i) => i)); // open all by default
