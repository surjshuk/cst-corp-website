...
const Special = ({ tabData, className, progressMultiplierHorizontal = 0, progressMultiplierVertical = {small: 0, default: 0} }: SpecialProps) => {
  ...
  return (
    <section ref={sectionRef} className={`text-white ${className}`}>
      ...
      >
        {/* Tab Navigation */}
        <div className="absolute z-20 p-5 tablet:px-12 tablet:py-10 laptop:px-20 laptop:py-10 w-44 tablet:w-60 laptop:w-80">
          The majority of our team is dedicated to R&D
        </div>
        <div className="relative w-fit ml-auto z-20 p-5 tablet:px-12 tablet:py-10 laptop:px-20 laptop:py-10">
          ...
          <div
            className="w-10 h-0.5 bg-primary relative transition-all duration-50 ease-linear laptop:block hidden"
            style={{
        ...
        {tabData.map((tab, index) => (
          <div
            className={`bg-black h-screen absolute bottom-14 tablet:top-0 w-full flex flex-col justify-end ${activeTab === index ? "" : "-z-10"}`}
            key={`section-main-${tab.id}`}
          >
            
            <div
              className={`z-20 flex justify-end flex-end flex-col tablet:bottom-12 tablet:absolute space-y-4 tablet:space-y-16 px-5 py-10 tablet:px-12 laptop:px-20 laptop:pt-36 laptop:pb-12 laptop:inset-0 ${
                activeTab === index ? "opacity-100 animate-fadeIn" : "opacity-0 animate-fadeOut"
              }`}
              >
              <h4 className="text-[28px] leading-[44px] tablet:text-[60px] tablet:leading-[0px] laptop:w-1/2">
                {tab.heading}
              </h4>

              <div className="space-y-5 tablet:space-y-4 laptop:flex laptop:space-y-0 gap-4">
                <p className="text-sm laptop:w-1/4">{tab.description}</p>
                {tab.description2 && <p className="text-sm laptop:w-1/4">{tab.description2}</p>}
              </div>
            </div>
            ...
            <Video
                src={tab.video}
                className="object-cover tablet:h-screen"
            />
          </div>
        ))}
      </div>
