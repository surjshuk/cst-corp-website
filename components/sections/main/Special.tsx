...
const Special = ({ tabData, className, progressMultiplierHorizontal = 0, }: SpecialProps) => {
      ...
      >
        ...
        {tabData.map((tab, index) => (
         ...
         <div className="bg-gradient-to-t tablet:bg-black/50 from-black to-transparent absolute inset-0" />
            ...
            >
              <h4 className="text-[28px] leading-[44px] tablet:text-8xl laptop:w-1/2 text-wrap">
                {tab.heading}
              </h4>
              <div>

              <div className="laptop:flex flex justify-between border-t border-neutral-500 py-8">
                {tab.description.map((e,i)=>(
                  <p key={i} className="text-xs tablet:text-sm laptop:w-2/12 text-wrap">
                    {e}
                </p>
                ))}
